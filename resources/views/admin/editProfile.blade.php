...
@section('content')
    ...
    <form action="{{ route('admin.updateProfile', $admin->id) }}" method="POST">
            ...
            <label for="name" class="block text-gray-700 font-bold mb-2">Name:</label>
                <input type="text" name="name" id="name" class="form-control" value="{{ $admin->name }}" readonly>
            </div>

            <div class="mb-2">
                <label for="barangay" class="block text-gray-700 font-bold mb-2">Barangay:</label>
                <input type="text" name="barangay" id="barangay" class="form-control" value="{{ $admin->barangay }}">
            </div>

            <div class="mb-2">
                <label for="municipal" class="block text-gray-700 font-bold mb-2">Municipal:</label>
                <input type="text" name="municipal" id="municipal" class="form-control" value="{{ $admin->municipal }}">
            </div>

            <div class="mb-2">
                <label for="province" class="block text-gray-700 font-bold mb-2">Province:</label>
                <input type="text" name="province" id="province" class="form-control" value="{{ $admin->province }}">
            </div>

            <div class="mb-2">
                <label for="contact" class="block text-gray-700 font-bold mb-2">Contact Number:</label>
                <input type="text" name="contact" id="contact" class="form-control" value="{{ $admin->contact }}" placeholder="+63 XXX-XXXX-XXX">
            </div>


            <div class="mb-2">
            <label for="email" class="block text-gray-700 font-bold mb-2">Email:</label>
                <input type="email" name="email" id="email" class="form-control" value="{{ $admin->email }}" readonly>
            </div>
