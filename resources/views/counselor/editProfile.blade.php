...
@section('content')
    ...
    <form action="{{ route('counselor.updateProfile', $counselor->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-2">
            <label for="name" class="block text-gray-700 font-bold mb-2">Name:</label>
                <input type="text" name="name" id="name" class="form-control" value="{{ $counselor->name }}" readonly>
            </div>

            <div class="mb-2">
                <label for="barangay" class="block text-gray-700 font-bold mb-2">Barangay:</label>
                <input type="text" name="barangay" id="barangay" class="form-control" value="{{ $counselor->barangay }}">
            </div>

            <div class="mb-2">
                <label for="municipal" class="block text-gray-700 font-bold mb-2">Municipal:</label>
                <input type="text" name="municipal" id="municipal" class="form-control" value="{{ $counselor->municipal }}">
            </div>

            <div class="mb-2">
                <label for="province" class="block text-gray-700 font-bold mb-2">Province:</label>
                <input type="text" name="province" id="province" class="form-control" value="{{ $counselor->province }}">
            </div>

            <div class="mb-2">
                <label for="contact" class="block text-gray-700 font-bold mb-2">Contact Number:</label>
                <input type="text" name="contact" id="contact" class="form-control" value="{{ $counselor->contact }}" placeholder="+63 XXX-XXXX-XXX">
            </div>


            <div class="mb-2">
            <label for="email" class="block text-gray-700 font-bold mb-2">Email:</label>
                <input type="email" name="email" id="email" class="form-control" value="{{ $counselor->email }}" readonly>
            </div>
            ...
            <div class="mb-2">
                ...
                <div class="input-group">
                    <input type="password" name="password" id="password" class="form-control" value="{{ $counselor->password }}">
